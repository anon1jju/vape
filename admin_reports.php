    ...
    if (is_array($block) && isset($block['items']) && is_array($block['items'])) {
      ...
      if ($blockTotal <= 0) {
        foreach ($block['items'] as $r) {
          ...
          $blockTotal += to_int($r['harga_jual'] ?? 0) * max(1, to_int($r['jumlah'] ?? 0));
        }
      }
      ...
      foreach ($block['items'] as $r) {
        ...
        if (isset($r['diskon_item'])) {
          ...
          if ($idx === $itemCount) {
            ...
            $di = (int)round($blockDiskon * $subtotalR / $blockTotal);
            $r['_diskon_item'] = $di;
            $sisaDiskon -= $di;
          }
        } else {
...
foreach ($filtered as $r) {
  ...
  $labaRaw = $labaPer * max(1, $qty);
  $sumLaba += max(0, $labaRaw - $diskonItem);
          ...
          <tbody>
            <?php foreach ($filtered as $r): ?>
              <?php
                ...
                $labaPer = to_int($r['laba_per_produk'] ?? 0);
                $laba = max(0, ($labaPer * max(1, $qty)) - $diskonItem);
